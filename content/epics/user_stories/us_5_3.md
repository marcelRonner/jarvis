---
title: "US-5.3 – Show Account Balance on Receipt"
linkTitle: "US-5.3 Show Account Balance on Receipt"
type: user-story
description: "As a Customer, I MUST see the current Account balance on every printed Receipt so that I always know my remaining funds after a Transaction."
weight: 16
tags: [atm, customer, account, session, transaction, receipt, r2-core-transactions, kano-basic]
sources:
  - id: epics
    resource: /epics/epics.md
  - id: print-receipt
    resource: /use_case/print_receipt/printReceipt.md
generated:
  by: jarvis/1.0
  at: 2026-09-12T21:29:07Z
---

# US-5.3 – Show Account Balance on Receipt

## Card

As a **Customer**, I **MUST** see the current Account balance on every printed Receipt so that I always know my remaining funds after a Transaction.

## Conversation

- Whenever the Customer opts to print a Receipt (BP step 5.3), the ATM retrieves the current Account `balance` and includes it in the Receipt `content`.
- This applies to every Transaction type: `WITHDRAWAL`, `BALANCE_INQUIRY`, and `TRANSFER`.
- Corresponds to activity diagram steps in [Print Receipt](../../use_case/print_receipt/printReceipt.md): "ATM retrieves current Account balance" → "ATM generates Receipt with Transaction details and Account balance".[^print-receipt]
- KANO: **Basic** — Customers expect their remaining balance on a receipt; its absence would cause dissatisfaction.

## Confirmation

- **Given** a Transaction with status `COMPLETED` exists in the current Session
  **When** the Customer chooses to print a Receipt
  **Then** the Receipt `content` includes the current Account `balance` alongside the Transaction details.

[^print-receipt]: [Print Receipt](../../use_case/print_receipt/printReceipt.md)
